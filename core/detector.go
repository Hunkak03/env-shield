...
func DetectEntropySecrets(filePath, line string, lineNum int, threshold float64, minLen int) []Finding {
	if IsIgnoredLine(line) {
		return nil
	}
...
func DetectForbiddenFile(filePath string) []Finding {
...
func ScanStagedFiles(cfg *Config) ([]Finding, error) {
...
func worker(jobs <-chan FileJob, results chan<- FileResult, cfg *Config) {
	for job := range jobs {
		findings, err := analyzeFile(job.Path, cfg)
		results <- FileResult{
			Index:    job.Index,
			Findings: findings,
			Err:      err,
...
func analyzeFile(filePath string, cfg *Config) ([]Finding, error) {
	// Layer 3: Check forbidden files first (fast path)
	forbiddenFindings := DetectForbiddenFile(filePath)
	if len(forbiddenFindings) > 0 {
		return forbiddenFindings, nil // Skip content for forbidden files
	}
	...
	for scanner.Scan() {
		...
		findings = append(findings, DetectEntropySecrets(filePath, line, lineNum, cfg.EntropyThreshold, cfg.MinSecretLength)...)
	}
